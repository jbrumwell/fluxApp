...
FluxAppContext.prototype._getAppContext = function _getAppContext(request, options) {
  var router = this._fluxApp.getRouter();
  var route = router.getRouteById(request.routeId);

  if (options.rehydrate && typeof options.state === 'object') {
  ...
  return new Promise(function renderContext(resolve, reject) {
    ...
    if (options.async) {
      result = route.loader(request, this).then(function() {
        return {
          Element: Element,
          state: options.dehydrate ? context.dehydrate() : {},
        }
      });
    } else {
      result = {
...
FluxAppContext.prototype.render = function render(path, options) {
  var self = this;
  var router = this._fluxApp.getRouter();
  var request = typeof path === 'object' ? router.build(path, options) : path;

  return request ? this._getAppContext(request, options || {}) : Promise.resolve({
    element: false,
    state: {}
  });

  return route.loader(request, this).then(function() {
    var markup = React.renderToString(Wrapper({
      handler: route.handler,
      context: context,
      params: routeRequest.params,
      query: routeRequest.query,
