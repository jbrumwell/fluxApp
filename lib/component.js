  ...
  _createClass(fluxappComponent, [{
    ...
    value: function _initActions(actions) {
      ...
      if (actions) {
        ...
        if (_lodash2['default'].size(this._actionMap)) {
          this._dispatchToken = fluxapp.getDispatcher().register(this.onDispatch.bind(this), _lodash2['default'].keys(this._actionMap));
        }
      }
    }
    ...
    value: function _bindActions(actionTypes, cb) {
      ...
      actionTypes.forEach(function (actionType) {
        ...
        _this3._actionMap[key] = cb.bind(_this3);
      });
    }
