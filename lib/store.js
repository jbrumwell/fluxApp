...
BaseStore.prototype._initDispatcher = function initDispatcher() {
  var Dispatcher = this._fluxApp.getDispatcher();
...
BaseStore.prototype.getStore = function getStore(name) {
  return this._fluxApp.getStore(name);
},
 ...
 * @param {Object} state
 */
BaseStore.prototype.replaceState = function replaceState(state) {
  this.state = state;
...
module.exports = function createStore(name, spec) {
  ...
  StoreWithSpec.prototype.clone = function clone(context) {
    return new StoreWithSpec(spec, context);
  };
