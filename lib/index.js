...
function navigateAway(context, e) {
  var routeStore = context.getRouter().getStore().state;
  var historySize = routeStore.history.length;
  var routeState = routeStore.current;

  if (historySize > 1 && routeState.path) {
...
FluxApp.prototype.request = function request(route, options) {
  ...
  context.getRouter().go(route.path);
...
FluxApp.prototype.fetch = function fetch(requestData) {
  return this._isoFetch.request(requestData);
};

FluxApp.prototype.setPlatform = function setPlatform(platform, options) {
  options = options || {};
  this._platformOptions = options;

  this._router = new Router(this);
  this._isoFetch = new IsoFetch(options.fetch, options.transports);
